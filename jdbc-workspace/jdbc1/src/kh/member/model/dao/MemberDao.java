...
//			6. 연결 종료
	...
	public int updateMember(Member e,int member_no) {
		int result = 0;
		/*
		 * create table member (
    member_no number primary key,
    ...
    enroll_date date default sysdate
		 ...
		 */
		String sql = String.format("update member set member_id = \'%s\',member_pw = \'%s\',member_name = \'%s\',age = \'%d\', gender = \'%c\', phone = \'%s\' where member_no = \'%d\'", 
				e.getMemberId(),e.getMemberPw(),e.getMemberName(),e.getAge(),e.getGender(),e.getPhone(),member_no);
		Connection conn = null;
		Statement stmt = null;
